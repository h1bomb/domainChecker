...
Checker.prototype.wgRun = function(num) {
    ...
    function _check() {
        if (i < words.length) {
            ...
            that.check(domains).then(function(data) {
                data.module.forEach(function(m) {
                    if (m.avail) {
                        console.log(m);
                    }
                });
...
Checker.prototype.check = function(domain) {
    var token = genToken();

    return req.getAsync(TOKEN_REG_URL + token).then(function() {
        return req.getAsync(DEFAULT_CHECK_URL + '?callback=' + DEFAULT_METHOD + '&domain=' + domain + '&token=' + token)
    }).then(function(args) {
        return jsonp.parseAsync(DEFAULT_METHOD, args[1]);
    }).then(function(data) {
        if (data) {
            var ds = new store();
            ds.insert(data);
